...
export Op_name="WJQWRT"                # 修改主机名称为OpenWrt-123(填0为不作修改)
...
export Customized_Information="WJQserver Studio build $(TZ=UTC-8 date "+%Y.%m.%d")"  # 个性签名,你想写啥就写啥，(填0为不作修改)
...
# 修改插件名字
sed -i 's/"KMS 服务器"/"KMS"/g' `egrep "KMS 服务器" -rl ./`
